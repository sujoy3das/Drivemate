...
def get_effective_permissions(item, user):
    """
    Finds the effective share permissions for a user on an item by checking the item,
    its parent folders, and any reshare chains. This is now an iterative implementation
    for better performance and to prevent recursion depth issues.
    ...
    current_item = item
    while current_item:
        share = get_share_permissions(current_item, user)
        if share:
            # A share record was found for the user on this item. Now we must validate it.
            if not share.reshared_by_id:
                # This is a direct share from the owner. Access is granted.
                return True, share.can_download, share.can_copy, share.can_reshare
            else:
                # This is a reshare. We must validate the entire chain.
                # Does the person who reshared it to us still have reshare permission?
                resharer = db.session.get(User, share.reshared_by_id)
                if resharer:
                    # We check the resharer's permissions on the *same item* we found the share on.
                    # We call the main function again for the resharer. This creates a clean,
                    # recursive validation of the chain without complex loops.
                    has_access, _, _, can_reshare = get_effective_permissions(current_item, resharer)
                    if has_access and can_reshare:
                        # The chain is valid. Grant access with the permissions from this share record.
                        return True, share.can_download, share.can_copy, share.can_reshare

        # If no valid share was found on the current item, move to its parent.
        if isinstance(current_item, File):
            current_item = current_item.folder
        elif isinstance(current_item, Folder):
            current_item = current_item.parent
        else:
            # Should not happen with valid File/Folder objects
            break
...
def convert_to_hls(input_path, output_folder):
    """Converts a video file to HLS format (playlist and segments)."""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    
    output_playlist = os.path.join(output_folder, 'playlist.m3u8')
    ...
    try:
        ...
        command = [
            ...
            '-hls_time', '10',
            '-hls_list_size', '0',
            '-hls_segment_filename', os.path.join(output_folder, 'segment%03d.ts'),
            '-start_number', '0',
            output_playlist
        ]
        subprocess.run(command, check=True, capture_output=True, text=True)
        app.logger.info(f"Successfully converted {os.path.basename(input_path)} to HLS in {output_folder}")
        return True
    except subprocess.CalledProcessError as e:
        app.logger.error(f"HLS conversion failed for {os.path.basename(input_path)}. Stderr: {e.stderr}")
        return False
    except FileNotFoundError:
        app.logger.error("ffmpeg command not found for HLS conversion.")
        return False

def is_hls_conversion_running(file_id):
    """Check if an HLS conversion lock file exists."""
    lock_file = os.path.join(HLS_FOLDER, str(file_id), "convert.lock")
...
def index(folder_id=None):
    ...
    if folder_id:
        ...
        # If sharer_id is present, we are navigating a shared structure.
        # Build a breadcrumb path that respects this shared context.
        if sharer_id and folder.user_id != current_user.id:
            temp_folder = folder
            sharer = db.session.get(User, sharer_id)
            # Find the highest-level shared ancestor in this path
            shared_ancestor = folder
            while shared_ancestor.parent and has_access(shared_ancestor.parent, current_user):
                shared_ancestor = shared_ancestor.parent
            # Build breadcrumbs from the shared ancestor down
            while temp_folder and temp_folder.id != shared_ancestor.parent_id:
        ...
        # If we are in a shared context, prepend the 'Shared' and 'Sharer' breadcrumbs
        if sharer_id and folder.user_id != current_user.id:
            sharer = db.session.get(User, sharer_id)
            breadcrumbs.insert(0, {'name': f"{sharer.name}", 'url': url_for('shared_items', sharer_id=sharer.id)})
            breadcrumbs.insert(0, {'name': 'Shared', 'url': url_for('shared_items')})
            breadcrumbs.insert(0, {'name': 'Home', 'url': url_for('index')})
...
def preview_file(file_id):
    ...
    # If the user is navigating within a shared folder, the primary access check
    # should be on the folder itself to ensure permission inheritance.
    if file.folder:
        if has_access(file.folder, current_user):
            access_granted = True
    
    # If not in a folder or if the folder check fails, fall back to checking the file directly.
    if not access_granted:
        access_granted = has_access(file, current_user)
...
def share_with_user():
    ...
    # If a folder is being shared, we don't need to create separate share records for items inside it.
    # 1. Get all folders being shared.
    shared_folders = Folder.query.filter(Folder.id.in_(folder_ids)).all()
    all_descendant_folder_ids = set(folder_ids)

    # 2. Recursively find all sub-folder IDs within the top-level shared folders.
    folders_to_process = list(shared_folders)
    while folders_to_process:
        current_folder = folders_to_process.pop(0)
        for child in current_folder.children:
            all_descendant_folder_ids.add(child.id)
            folders_to_process.append(child)

    # 3. Filter the file list to only include files that are NOT in any of the shared folders or their subfolders.
    final_file_ids_to_share = []
    for file_id in file_ids:
        file = db.session.get(File, file_id)
        # Only add the file to the share list if its parent folder is NOT in the set of all shared folders and their descendants.
        if file and file.folder_id not in all_descendant_folder_ids:
